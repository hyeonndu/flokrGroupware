...
*   전자결재 문서함 관리
    - 상신, 수신, 완료, 임시저장 문서함으로 구분된 결재 문서 관리  
    - **State Pattern** 기반 결재 흐름 구현 → 승인, 반려, 진행 상태 전환 로직을 객체로 분리하여 유지보수성과 확장성 강화  
    - 결재 상태 변경 시 해당 문서함으로 자동 이동 (예: 상신 → 수신, 모든 결재 완료 → 완료 문서함)  
    - 각 문서함에서 상세보기, 검색 기능 제공  
*   Redis 캐싱 적용
    - 수신문서함: 반복 조회 시 캐싱을 통한 조회 성능 최적화  
    - 임시저장함: 작성 중 페이지 이탈·서버 재시작 시 데이터 복원 가능, TTL 만료 시 자동 삭제로 데이터 관리 효율성 향상
*   결재 문서 작성
    - 휴가신청서, 지출결의서, 출장신청서, 구매요청서 4종의 결재양식 제공  
    - 선택한 양식에 따라 입력 폼과 필수 항목이 자동 변경  
    - 작성한 문서 저장 시 결재 상태 초기화(기안 상태) 및 문서함 반영      
*   결재선 지정
    - 결재자 선택 기능 구현  
    - 사원 목록에서 다중 선택 가능하며 지정 순서에 따라 결재 진행  
*   일정 관리
    - FullCalendar 기반 월간·주간·일간 뷰 지원  
    - 일정 등록, 수정, 삭제 기능 제공  
    - 중요도(HIGH)에 따른 시각적 강조 표시  
    - 카테고리(개인/팀/회사/기타)별 색상 구분 및 필터링  
